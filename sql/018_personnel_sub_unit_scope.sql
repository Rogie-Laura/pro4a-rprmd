-- RHQ/PHQ admins: unit scope includes hyphenated sub-units (ORPRMD → ORPRMD-RPHAS).
...
create or replace function public.list_personnel_rprmd_paged(
  ...
  p_include_sub_units boolean default false
)
returns json
language plpgsql
stable
...
declare
  ...
  v_include_sub_units boolean := coalesce(p_include_sub_units, false);
  ...
  with filtered as (
    ...
    where
      ...
      and (
        ...
        or (
          v_include_sub_units
          and lower(btrim(coalesce(p.station, ''))) like lower(v_station) || '-%'
        )
      )
      and (
        p_view = 'all'
...
revoke execute on function public.list_personnel_rprmd_paged(text, text, text, int, int, text, text, boolean) from anon, authenticated;
grant execute on function public.list_personnel_rprmd_paged(text, text, text, int, int, text, text, boolean) to service_role;
